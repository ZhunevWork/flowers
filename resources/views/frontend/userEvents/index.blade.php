        ...
        <div class="col-md-12">
            @can('user_event_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-events.create') }}">
                            ...
                            <tbody>
                                @foreach($userEvents as $key => $userEvent)
                                    <tr data-entry-id="{{ $userEvent->id }}">
                                        ...
                                        <td>
                                            @can('user_event_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-events.show', $userEvent->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_event_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-events.edit', $userEvent->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_event_delete')
                                                <form action="{{ route('frontend.user-events.destroy', $userEvent->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_event_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-events.massDestroy') }}",
