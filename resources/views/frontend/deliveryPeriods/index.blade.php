        ...
        <div class="col-md-12">
            @can('delivery_period_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.delivery-periods.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $deliveryPeriod->id }}">
                                        ...
                                        <td>
                                            @can('delivery_period_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.delivery-periods.show', $deliveryPeriod->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('delivery_period_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.delivery-periods.edit', $deliveryPeriod->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('delivery_period_delete')
                                                <form action="{{ route('frontend.delivery-periods.destroy', $deliveryPeriod->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('delivery_period_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.delivery-periods.massDestroy') }}",
