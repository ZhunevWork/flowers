...
@can('element_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.elements.create') }}">
                ...
                <tbody>
                    @foreach($elements as $key => $element)
                        <tr data-entry-id="{{ $element->id }}">
                            ...
                            <td>
                                @can('element_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.elements.show', $element->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('element_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.elements.edit', $element->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('element_delete')
                                    <form action="{{ route('admin.elements.destroy', $element->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('element_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.elements.massDestroy') }}",
