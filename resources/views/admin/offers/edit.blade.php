    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.offers.update", [$offer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cart') ? 'is-invalid' : '' }}" type="number" name="cart" id="cart" value="{{ old('cart', $offer->cart) }}" step="1" required>
                @if($errors->has('cart'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('count') ? 'is-invalid' : '' }}" type="number" name="count" id="count" value="{{ old('count', $offer->count) }}" step="1">
                @if($errors->has('count'))
                    <div class="invalid-feedback">
                        {{ $errors->first('count') }}
                    </div>
