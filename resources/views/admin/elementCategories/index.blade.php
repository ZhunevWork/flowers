...
@can('element_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.element-categories.create') }}">
                        ...
                        <tr data-entry-id="{{ $elementCategory->id }}">
                            ...
                            <td>
                                @can('element_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.element-categories.show', $elementCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('element_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.element-categories.edit', $elementCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('element_category_delete')
                                    <form action="{{ route('admin.element-categories.destroy', $elementCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('element_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.element-categories.massDestroy') }}",
